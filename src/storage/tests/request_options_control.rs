...
mod tests {
    ...
    #[tokio::test]
    async fn get_bucket_with_custom_and_system_headers() -> anyhow::Result<()> {
        let mut mock = MockStorage::new();
        mock.expect_get_bucket()
            .withf(|req| {
                let tracking_ok = req
                    .metadata()
                    .get("x-client-tracking-id")
                    .and_then(|v| v.to_str().ok())
                    == Some("req-123");
                let quota_ok = req
                    .metadata()
        ...
        let _ = client
            .get_bucket()
            .set_name("projects/_/buckets/my-bucket")
            .with_custom_header("x-client-tracking-id", "req-123")
            .with_quota_project(PROJECT_NAME)
            .send()
            .await?;
    ...
    #[tokio::test]
    async fn get_bucket_system_header_precedence() -> anyhow::Result<()> {
        let mut mock = MockStorage::new();
        mock.expect_get_bucket()
            .withf(|req| {
                req.metadata()
        ...
        let _ = client
            .get_bucket()
            .set_name("projects/_/buckets/my-bucket")
            .with_custom_header("x-goog-user-project", "fake_project")
            .with_quota_project(PROJECT_NAME)
            .send()
            .await?;
